...
path_data <- "E:/Allie/_FireFX/FFI Data Management/Exports from FFI/GRCA_FMH/2025-01-23/"
path_errors <- "C:/Users/allie/OneDrive/Desktop/R Projects/GRCA/errors/"
```
...
-   Height taller than 2 meters entered.

-   Height entered for species that is not the tallest species at a point. The tallest species should have Order = 1. All shorter species should have Order \> 1 and no height.

-   Height entered for a substrate.
...
```{r}
errors_HerbsPoints_Height <- unique(rbind())
...
-   Incorrect order numbers entered or order number missing.

-   Duplicate entries exist for a single order number.

-   Substrates recorded beneath plants or multiple substrates recorded at a point.
...
```{r}
errors_HerbsPoints_Order <- unique(rbind())
...
# 
# # Identify errors
# errors_HerbsPoints_Points_Missing <- qaqc(data, query, query_message, values_check)
```

#### Combine Point Errors

```{r}
errors_HerbsPoints_Point <- unique(rbind())
...
-   Check that Status = L or D.

-   On the Filter tab, under Species, click on Biological Not Filtered until it changes to Biological Only. Apply Selections, Sort by Status, check for annual herbaceous species with Status = D (entirely new query?).
...
data <- HerbsPoints_data
query <- "Cvr Pts Tape" 
query_message <- paste0("Transect = ", data$Transect, ". ", "Point = ", data$Point, ". ", "Tape")
