...
```{r}
data_temp <- Trees_data_CrossYear %>% 
  group_by(MacroPlot.Name, Date) %>% 
  filter(Status == "L") %>% 
  mutate(Tree_Count = n_distinct(unique(TagNo))) %>%
  get_dupes(MacroPlot.Name, QTR, SubFrac, TagNo, Status, DBH, Ht, CrwnCl, LiCrBHt, CharHt, ScorchHt, CrScPct, DamCd1, DamCd2, DamCd3, DamCd4, DamCd5) %>%
  ungroup()

data_temp1 <- Trees_data_CrossYear %>% 
  group_by(MacroPlot.Name, Date) %>% 
    filter(Status == "L") %>% 
  mutate(Tree_Count = n_distinct(unique(TagNo))) %>%
  get_dupes(MacroPlot.Name, QTR, SubFrac, TagNo, Status, DBH, Ht, CrwnCl, LiCrBHt, CharHt, ScorchHt, CrScPct, DamCd1, DamCd2, DamCd3, DamCd4, DamCd5) %>%
  ungroup() %>% 
  select(MacroPlot.Name, Date, Tree_Count, dupe_count) %>% 
  arrange(MacroPlot.Name, Date) %>% 
  unique()


data_temp <- merge(data_temp1, data_temp2, by = c("MacroPlot.Name", "Date"), all.x = T)
...
query_message <- paste0("Duplicates")
values_data <- data$dupe_count
values_valid <- data$Tree_Count
values_check <- values_data != values_valid

# Identify errors
errors_Trees_Duplicate <- qaqc_CrossYear(data, query, query_message, values_check) %>% 
