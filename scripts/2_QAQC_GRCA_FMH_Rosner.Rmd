...
#PostBurn_AllYears_all <- read.csv(paste0(path_data, "Post Burn Severity_XPT.csv"))
```
...
# Clean up data frame
Fuels1000_AllYears_all <- Fuels1000_AllYears_all %>%
...
Fuels1000_AllYears_header <- Fuels1000_AllYears_all %>%
  filter(!is.na(Visited))
```
...
# Clean up data frame 
FuelsDuffLitt_AllYears_all <- FuelsDuffLitt_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE)
...
# Filter for only headers 
FuelsDuffLitt_AllYears_header <- FuelsDuffLitt_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
FuelsFine_AllYears_all <- FuelsFine_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE)
...
# Filter for only headers 
FuelsFine_AllYears_header <- FuelsFine_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
HerbsPoints_AllYears_all <- HerbsPoints_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE) 
...
# Filter for only headers 
HerbsPoints_AllYears_header <- HerbsPoints_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
HerbsObs_AllYears_all <- HerbsObs_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE) 
...
# Filter for only headers 
HerbsObs_AllYears_header <- HerbsObs_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
Shrubs_AllYears_all <- Shrubs_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE)
...
# Filter for only headers 
Shrubs_AllYears_header <- Shrubs_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
Seedlings_AllYears_all <- Seedlings_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE) 
...
# Filter for only headers 
Seedlings_AllYears_header <- Seedlings_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
# Clean up data frame 
Trees_AllYears_all <- Trees_AllYears_all %>%   
  # Separate Date column into Date and Time columns   
  separate(Date, sep = " ", into = c("Date", "Time")) %>%   
  # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
  separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE) 
...
# Filter for only headers 
Trees_AllYears_header <- Trees_AllYears_all %>%   
  filter(!is.na(Visited))
```
...
```{r}
# # Ensure blanks in Visited column are NA 
# PostBurn_AllYears_all$Visited[PostBurn_AllYears_all$Visited==""] <- NA  
# 
# # Clean up data frame 
# PostBurn_AllYears_all <- PostBurn_AllYears_all %>%   
#   # Separate Date column into Date and Time columns   
#   separate(Date, sep = " ", into = c("Date", "Time")) %>%   
#   # Separate MacroPlotName to get Monitoring type (or export data with Monitoring Type already included)   
#   separate(MacroPlot.Name, sep = " ", into = c("MonitoringType", "Plot"), remove = FALSE)
# 
# # Filter for only data 
# PostBurn_AllYears_data <- PostBurn_AllYears_all %>%   
#   filter(is.na(Visited))
# 
# # Filter for only headers 
# PostBurn_AllYears_header <- PostBurn_AllYears_all %>%   
#   filter(!is.na(Visited))
```
