...
#### Combine Dead Tree Errors

```{r}
# Combine
errors_Trees_DD <- rbind(errors_Trees_DD_DBH, errors_Trees_DD_Ht, 
                         errors_Trees_DD_LiCrBHt, errors_Trees_Dead_LiCrBHt)

remove(errors_Trees_DD_DBH, errors_Trees_DD_Ht, errors_Trees_DD_LiCrBHt, errors_Trees_Dead_LiCrBHt)
```
...
query <- "Trees Non-Post Severity FMH CharHt" 
...
data <- Trees_data %>% 
  filter(!Monitoring.Status %in% c("01Post", "02Post", "03Post", "04Post"))
query <- "Trees Non-Post Severity FMH ScorchHt" 
...
data <- Trees_data %>% 
  filter(!Monitoring.Status %in% c("01Post", "02Post", "03Post", "04Post"))
query <- "Trees Non-Post Severity FMH CrScPct" 
...
-   Filter to include all POST reads (01Post, 02Post, etc). Check that all rows with CrwnCl = DD, BBD, or CUS have CharHt = blank.

```{r}
data <- Trees_data %>% 
  filter(Monitoring.Status %in% c("01Post", "02Post", "03Post", "04Post"),
         CrwnCl %in% c("DD", "BBD", "CUS"))
query <- "Trees Post Char FMH DD" 
query_message <- "Char Height"  
values_data <- data$CharHt
values_valid <- ""
values_check <- !values_data == values_valid
...
-   Filter to include all POST reads (01Post, 02Post, etc). Check that CharHt is not blank or excessively high for standing trees. If CharHt is blank, TagNo should be blank
...
data <- Trees_data %>% 
  filter(Monitoring.Status %in% c("01Post", "02Post", "03Post", "04Post"),
         !CrwnCl %in% c("DD", "BBD", "CUS"),
         # CharHt is blank when TagNo = 0. OK
         !TagNo == 0)
query <- "Trees Post Char FMH Standing" 
query_message <- "Char Height"  
values_data <- data$CharHt
values_valid <- max(14)
values_check <- !values_data < values_valid %>% replace_na(TRUE)
...
-   Filter to include all POST reads (01Post, 02Post, etc).

-   Check that all rows with CrwnCl = BAD, BBD, or CUS have CrScPct = blank. (CS? LBS?)

-   Check that all rows with CrwnCl = DD have CrScPct = blank or 100. (RS?)

<!-- -->

-   Check that CrScPct ≤ 100.

-   Check that CrScPct is not blank for live trees. For rows that have a CrScPct = blank, either TagNo should be blank OR Status should be D.
...
errors_Trees <- rbind(errors_Trees_Header, errors_Trees_CrwnCl, errors_Trees_DamCd,
                      errors_Trees_DBH, errors_Trees_DD, errors_Trees_Duplicate, 
