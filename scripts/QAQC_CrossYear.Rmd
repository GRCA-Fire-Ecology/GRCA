...
```{r}
# Blank data frame
errors_blank_CrossYear <- data.frame("SavedQuery" = "", "MacroPlot.Name" = "", "TagNo" = "", "Error" = "", "Fixed" = "", "Explanation" = "", "Queryers" = "")
...
```{r}
# Load in data
Fuels1000_all <- read.csv(paste0(path_data_FMH, "Surface Fuels - 1000Hr_XPT.csv"), quote = "")
FuelsFine_all <- read.csv(paste0(path_data_FMH, "Surface Fuels - Fine_XPT.csv"), quote = "")
...
# Remove extra data
remove(Trees_all_RAP_N, Trees_all_RAP_S, Trees_all_IM_M, Trees_all_IM_P)
```
...
Trees_all <- Trees_all_IM

path_errors_name <- "GRCA FMH QAQC Tree Errors"
```
...
remove(Fuels1000_data_Slope)

# Set parameters 
data <- merge(Fuels1000_data, data_temp, all = T)
query <- "Fuel 1000 Slope" 
query_message <- paste0("Transect = ", data$Transect, ". ", "Slope = ", data$Slope, ". ", "Conflicting slopes")
values_data <- data$Slope_Count
values_valid <- 1
values_check <- (values_data == values_valid) %>%  replace_na(TRUE)

# Identify errors
errors_Fuels1000_Slope_CrossYear <- qaqc(data, query, query_message, values_check) %>% 
  arrange(MacroPlot.Name, Error)
```
...
```{r}
# Find data with multiple slopes
FuelsFine_data_Slope <- FuelsFine_data %>% 
  group_by(MacroPlot.Name, Transect) %>% 
  mutate(Slope_Count = n_distinct(unique(Slope))) %>% 
  ungroup() %>% 
  arrange(MacroPlot.Name, Date, Transect)

data_temp <- FuelsFine_data_Slope %>% 
  filter(Slope_Count > 1) %>%
  group_by(MacroPlot.Name, Transect) %>% 
  mutate(Slope_Change = ifelse(duplicated(Slope), "N", "Y")) %>%
  filter(Slope_Change == "Y") %>%
...
remove(FuelsFine_data_Slope)

# Set parameters 
data <- merge(FuelsFine_data, data_temp, all = T)
...
# Identify errors
errors_FuelsFine_Slope_CrossYear <- qaqc(data, query, query_message, values_check) %>% 
  arrange(MacroPlot.Name, Error)
```
...
FuelsFine_data_Azimuth <- FuelsFine_data %>% 
  group_by(MacroPlot.Name, Transect) %>% 
  mutate(Azimuth_Count = n_distinct(unique(Azimuth))) %>% 
  ungroup() %>% 
  arrange(MacroPlot.Name, Date, Transect)

data_temp <- FuelsFine_data_Azimuth %>% 
...
remove(FuelsFine_data_Azimuth)

# Set parameters 
data <- merge(FuelsFine_data, data_temp, all = T) %>% 
  filter(AzimuthDiff >= 25)
query <- "Fuel Fine Azimuth" 
query_message <- paste0("Transect = ", data$Transect, ". ", "Azimuth = ", data$Azimuth, ". ", "Conflicting azimuths")
values_data <- data$Azimuth_Count
values_valid <- 1
values_check <- (values_data == values_valid) %>%  replace_na(TRUE)

# Identify errors
errors_FuelsFine_Azimuth_CrossYear <- qaqc(data, query, query_message, values_check) %>% 
  arrange(MacroPlot.Name, Error)
```
...
# Identify errors
errors_Trees_CrossYear_Species_1 <- qaqc_CrossYear(data, query, query_message, values_check)
...
for (i in 2:nrow(errors_Trees_CrossYear)) {
  if (errors_Trees_CrossYear[i, "TagNo"] != errors_Trees_CrossYear[i - 1, "TagNo"]) {
    errors_Trees_CrossYear_blanks <- rbind(errors_Trees_CrossYear_blanks, errors_blank_CrossYear, errors_Trees_CrossYear[i, ])
  } else {
...
errors_Trees_CrossYear_blanks <- condformat(errors_Trees_CrossYear_blanks[1:(nrow(errors_Trees_CrossYear_blanks)), 1:7]) %>%
  rule_fill_discrete(columns = 1:7,
                     ...
                     na.value = "#FFFFFF")
print(errors_Trees_CrossYear_blanks)
```
...
# Formatted
condformat2excel(errors_Trees_CrossYear_blanks, paste0(path_errors, "errors_CrossYear_formatted.xlsx"), sheet_name = path_errors_name, overwrite_wb = TRUE)
```
