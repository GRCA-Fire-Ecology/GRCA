...
Fuels1000_data <- Fuels1000_all %>%
  filter(is.na(Visited), Year %in% c(2022, 2023, 2024))
...
errors_Fuels1000 <- rbind(errors_Fuels1000_DecayCl, errors_Fuels1000_FuelConst, 
                          errors_Fuels1000_NumTran, errors_Fuels1000_TranLen, 
                          errors_Fuels1000_LogNo, errors_Fuels1000_TransNum, 
                          errors_Fuels1000_Slope, errors_Fuels1000_Dia)

remove(errors_Fuels1000_DecayCl, errors_Fuels1000_FuelConst, 
       errors_Fuels1000_NumTran, errors_Fuels1000_TranLen, errors_Fuels1000_LogNo,
...
query <- "Fuel Fine Header I&M NumTran" 
query_message <- "Number of Transects"  
values_data <- data$NumTran  
values_valid <- 3    
values_check <- !values_data == values_valid
...
# Set parameters  
data <- FuelsFine_header
query <- "Fuel Fine Header I&M OneHrTranLen" 
...
# Set parameters  
data <- FuelsFine_header
query <- "Fuel Fine Header I&M TenHrTranLen" 
...
# Set parameters  
data <- FuelsFine_header
query <- "Fuel Fine Header I&M HunHrTranLen" 
...
```{r}
# Save to master error list 
errors_FuelsFine <- rbind(errors_FuelsFine_FuelConst, errors_FuelsFine_NumTran, errors_FuelsFine_OneHrTranLen, errors_FuelsFine_TenHrTranLen, errors_FuelsFine_HunHrTranLen, errors_FuelsFine_Hits, errors_FuelsFine_TransNum, errors_FuelsFine_Azimuth, errors_FuelsFine_Slope, errors_FuelsFine_OneHr,  errors_FuelsFine_TenHr, errors_FuelsFine_HunHr)
...
```{r}
# Save to master error list 
errors_Trees_Header <- rbind(errors_Trees_MacroPlotSize, errors_Trees_SnagPlotSize, errors_Trees_BrkPntDia)
...
-   Sort by DBH. If DBH = blank and Status = D, then check that CrwnCl = BBD, CUS, or DD
...
```{r}
Trees_data$LiCrBHt[is.na(Trees_data$LiCrBHt)] <- ""
...
```{r}
Trees_data$DBH[is.na(Trees_data$DBH)] <- ""
...
```{r}
Trees_data$Ht[is.na(Trees_data$Ht)] <- ""
...
Trees_data$Ht <- as.numeric(Trees_data$Ht)
...
```{r}
Trees_data$CharHt[is.na(Trees_data$CharHt)] <- ""
...
```{r}
Trees_data$ScorchHt[is.na(Trees_data$ScorchHt)] <- ""
...
```{r}
Trees_data$CrScPct[is.na(Trees_data$CrScPct)] <- ""
...
# Valid values were found by using the Rosner test on the full suite of data.
# Refer to the script 2_QAQC_Rosner.Rmd
Trees_data$CharHt <- as.numeric(Trees_data$CharHt)
...
# Valid values were found by using the Rosner test on the full suite of data.
# Refer to the script 2_QAQC_Rosner.Rmd
Trees_data$ScorchHt <- as.numeric(Trees_data$ScorchHt)
...
```{r}
Trees_data$CrScPct <- as.numeric(Trees_data$CrScPct)
...
errors_all <- rbind(errors_Fuels1000, errors_FuelsDuffLitt, 
                    errors_FuelsFine, errors_PostBurn, errors_Trees)
