...
values_valid <- 30
values_check <- (values_data == values_valid) %>% replace_na(FALSE)
...
# Identify errors
errors_Trees_Ht_LiCrBHt <- qaqc(data, query, query_message, values_check)
```
...
```{r}
# Combine
errors_Trees_CBH <- unique(rbind(errors_Trees_Dead_LiCrBHt, errors_Trees_Ht_LiCrBHt))

remove(errors_Trees_Dead_LiCrBHt, errors_Trees_Ht_LiCrBHt)
```
...
-   Filter by DBH = blank and Status = D. Check that CrwnCl = BBD, CUS, or DD
...
# Identify errors
errors_Trees_Height <- qaqc(data, query, query_message, values_check)
```
