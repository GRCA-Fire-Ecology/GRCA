...
sum_DisturbanceHistory <- sum_DisturbanceHistory %>% 
...
# add more interesting columns
MetadataReport <- MetadataReport %>% 
  
  # add park
  mutate(Park = "GRCA") %>% 
  relocate(Park, .after = AdministrationUnit_Name) %>% 
  
  # add park rim
  mutate(Park_Rim = ifelse(Purpose %in% c("PIED", "PIPO", "Moqui", "Picnic", "Quarry", "Tusayan Pueblo (Thinning)"), "South", "North")) %>% 
  relocate(Park_Rim, .after = PlotType_Name) %>% 
  
  # separate UV1 to get Burn Unit info
  separate(UV1, sep = " Project Unit", into = c("Burn_Unit"), remove = F) %>% 
  relocate(Burn_Unit, .after = Purpose) %>% 
  relocate(MetaData, .after = Burn_Unit) %>% 
  relocate(Type, .after = Park) %>% 
  
  # separate Macroplot to get the plot code and plot number
...
```{r}
graph_MetadataReport_fires <- graph_MetadataReport %>% 
  filter(PlotType_Name == "FMH") %>% 
  filter(!Macroplot %in% c("SHOS 01", "SHOS 02")) %>% 
  filter(Entry != "0") %>% 
  group_by(Burn_Unit, DisturbanceDate, DisturbanceDate_Year, SampleEventDate_Year, Park_Rim, MonStatus, Disturbance_Name, Disturbance_Desc, Disturbance_Severity) %>% 
  filter(DisturbanceDate_Year == SampleEventDate_Year) %>% 
  summarise(n_macroplot = n())
```

```{r}
graph_MetadataReport_fires %>% 
  filter(Park_Rim == "North") %>% 
