...
Fuels1000_all <- Fuels1000_all_FMH
FuelsFine_all <- FuelsFine_all_FMH
Trees_all <- Trees_all_FMH
...
Fuels1000_data_Slope <- Fuels1000_data_CrossYear %>% 
...
# Set parameters 
data <- merge(Fuels1000_data_CrossYear, data_temp, all = T)
query <- "Fuel 1000 Slope" 
query_message <- paste0("Slope = ", data$Slope, ". ", "Conflicting slopes")
values_data <- data$Slope_Count
values_valid <- 1
values_check <- (values_data == values_valid) %>%  replace_na(TRUE)

# Identify errors
errors_Fuels1000_Slope_CrossYear <- qaqc_fuels(data, query, query_message, values_check)
```
...
```{r}
# Find data with multiple slopes
FuelsFine_data_Slope <- FuelsFine_data_CrossYear %>% 
  group_by(MacroPlot.Name, Transect) %>% 
  mutate(Slope_Count = n_distinct(unique(Slope))) %>% 
  ungroup() %>% 
  arrange(MacroPlot.Name, Date, Transect)
...
# Set parameters 
data <- merge(FuelsFine_data_CrossYear, data_temp, all = T)
...
# Identify errors
errors_FuelsFine_Slope_CrossYear <- qaqc_fuels(data, query, query_message, values_check)
```
...
FuelsFine_data_Azimuth <- FuelsFine_data_CrossYear %>% 
  group_by(MacroPlot.Name, Transect) %>% 
...
query_message <- paste0("Azimuth = ", data$Azimuth, ". ", "Conflicting azimuths")
values_data <- data$Azimuth_Count
values_valid <- 1
values_check <- (values_data == values_valid) %>%  replace_na(TRUE)

# Identify errors
errors_FuelsFine_Azimuth_CrossYear <- qaqc_fuels(data, query, query_message, values_check)
```
