...
# Identify errors
errors_Fuels1000_CrossYear_Slope_1 <- qaqc_fuels(data, query, query_message, values_check)

# Consolidate duplicate species errors into one row
errors_Fuels1000_CrossYear_Slope <- errors_Fuels1000_CrossYear_Slope_1 %>%
...
# Identify errors
errors_FuelsFine_CrossYear_Slope_1 <- qaqc_fuels(data, query, query_message, values_check)

# Consolidate duplicate species errors into one row
errors_FuelsFine_CrossYear_Slope <- errors_FuelsFine_CrossYear_Slope_1 %>%
  separate(Error, sep = ". Conflicting slopes = Y", into = c("Error")) %>% 
  separate(Error, sep = "date = ", into = c("Test", "Date"), remove = F) %>% 
  arrange(MacroPlot.Name, Transect, Date) %>% 
  group_by(SavedQuery, MacroPlot.Name, Transect) %>% 
...
# Identify errors
errors_FuelsFine_CrossYear_Azimuth_1 <- qaqc_fuels(data, query, query_message, values_check)

# Consolidate duplicate species errors into one row
errors_FuelsFine_CrossYear_Azimuth <- errors_FuelsFine_CrossYear_Azimuth_1 %>%
