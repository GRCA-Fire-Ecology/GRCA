...
WACA_MacroplotReport_MetaData <- WACA_MacroplotReport_MetaData %>% 
  ...
  # Metadata
  mutate(MetaData = ifelse(MetaData == "", Macroplot, MetaData)) %>% 
...
sum_GRCA_MonStatus <- GRCA_MacroplotReport_SampleEvents %>% 
  select(MonStatusOrd, MonStatus) %>% 
  mutate(MonStatusOrd = NA)

sum_IM_MonStatus <- IM_MacroplotReport_SampleEvents %>% 
  select(MonStatusOrd, MonStatus) %>% 
  mutate(MonStatusOrd = NA)

sum_WACA_MonStatus <- WACA_MacroplotReport_SampleEvents %>% 
  select(MonStatusOrd, MonStatus) %>% 
  mutate(MonStatusOrd = NA)

# Combine Monitoring Statuses, keep only unique values
sum_MonStatus <- rbind(sum_GRCA_MonStatus, sum_IM_MonStatus, sum_WACA_MonStatus) %>% 
  unique() %>% 
  filter(MonStatus != "")

# Create column with consistent naming
sum_MonStatus <- sum_MonStatus %>% 
...
```{r}
sum_GRCA_DisturbanceHistory <- rbind(GRCA_Disturbance01, GRCA_Disturbance02, GRCA_Disturbance03, GRCA_Disturbance04)

sum_IM_DisturbanceHistory <- rbind(IM_Disturbance01)

sum_WACA_DisturbanceHistory <- rbind(WACA_Disturbance01)

#sum_DisturbanceHistory <- rbind(sum_GRCA_DisturbanceHistory, sum_IM_DisturbanceHistory, sum_WACA_DisturbanceHistory)
