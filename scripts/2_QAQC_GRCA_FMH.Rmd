...
query <- "Fuel 1000 Header TranLen NRim" 
...
query <- "Fuel 1000 Header TranLen SRim" 
query_message <- "Transect Length" 
values_data <- data$TranLen 
values_valid <- 100 
values_check <- values_data == values_valid
...
remove(errors_Trees_CrwnCl, errors_Trees_DamCd, errors_Trees_DBH, errors_Trees_CBH, errors_Trees_DD, errors_Trees_Duplicate, errors_Trees_Frac, errors_Trees_Header, errors_Trees_Height, errors_Trees_Complete, errors_Trees_NonPost_Severity, errors_Trees_Post_Char, errors_Trees_Post_ScorchHt, errors_Trees_Post_ScorchPct, errors_Trees_Status, errors_Trees_LifeForm, errors_Trees_UV)
```
...
query <- "Dens Quad Header Area 250" 
...
query <- "Dens Quad Header Area 50" 
query_message <- paste0("Area")
values_data <- data$Area
values_valid <- 50
values_check <- values_data == values_valid
...
data <- Seedlings_header %>% 
  filter(MacroPlot.Purpose %in% c("PIED", "PIPO", "PIPN"))
query <- "Dens Quad Header QuadWid 25" 
...
data <- Seedlings_header %>% 
  filter(MacroPlot.Purpose %in% c("PIEN", "PIAB"))
query <- "Dens Quad Header QuadWid 10" 
query_message <- paste0("Quad Width")
values_data <- data$QuadWid
values_valid <- 10
values_check <- values_data == values_valid
...
data <- Seedlings_header %>% 
  filter(MacroPlot.Purpose %in% c("PIED", "PIPO", "PIPN"))
query <- "Dens Quad Header QuadLen 10" 
...
data <- Seedlings_header %>% 
  filter(MacroPlot.Purpose %in% c("PIEN", "PIAB"))
query <- "Dens Quad Header QuadLen 5" 
query_message <- paste0("Quad Length")
values_data <- data$QuadLen
values_valid <- 5
values_check <- values_data == values_valid
...
```{r}
errors_Seedlings_Header <- unique(rbind(errors_Seedlings_Area_1, errors_Seedlings_Area_2, errors_Seedlings_QuadWid_1, errors_Seedlings_QuadWid_2, errors_Seedlings_QuadLen_1, errors_Seedlings_QuadLen_2, errors_Seedlings_NumTran, errors_Seedlings_NumQuadTran))

remove(errors_Seedlings_Area_1, errors_Seedlings_Area_2, errors_Seedlings_QuadWid_1, errors_Seedlings_QuadWid_2, errors_Seedlings_QuadLen_1, errors_Seedlings_QuadLen_2, errors_Seedlings_NumTran, errors_Seedlings_NumQuadTran)
```
...
```{r}
errors_Shrubs_Header <- unique(rbind())
...
-   Check that Count is not blank or 0 or excessively high
...
-   Check that Subbelt value is between 1 and 10
...
```{r}
errors_HerbsObs <- rbind(errors_HerbsObs_Cover, errors_blank, errors_HerbsObs_Complete, errors_blank, errors_HerbsObs_Status)

remove(errors_HerbsObs_Cover, errors_HerbsObs_Complete, errors_HerbsObs_Status)
```
