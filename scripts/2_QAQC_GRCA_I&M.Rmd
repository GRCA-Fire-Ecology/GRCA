...
# path_errors <- "C:/Users/alalor.NPS/OneDrive - DOI/Desktop/R/GRCA/output/I&M/M_errors/"
# path_data <- "C:/Users/allie/OneDrive/Desktop/R Projects/GRCA/data/qaqc/I&M/M_AllYears/"
# path_errors <- "C:/Users/allie/OneDrive/Desktop/R Projects/GRCA/output/I&M/M_errors/"
...
Fuels1000_all <- Fuels1000_all %>%
  ...
  select(!c("Month", "Day")) %>%
  # Add Monitoring Type
  separate(MacroPlot.Name, sep = "_", into = c("Park", "Name"), remove = FALSE)

verify <- Fuels1000_all$Name
verify <- as.data.frame(verify) %>% 
  filter(!is.na(verify))

Fuels1000_all <- Fuels1000_all %>%
  mutate(Name = ifelse(empty(verify) == FALSE, Name, Park)) %>% 
  separate(Name, sep = "(?=[[:digit:]])", into = c("MonitoringType", "Plot"), remove = FALSE) %>%
  select(!c("Park", "Plot")) %>% 
...
# Save as CSV or XLSX 
#path_errors
#write.csv(errors_all, paste0(path_errors, "errors_all_I&M.csv"), quote=FALSE, row.names = FALSE, na = "") 
write_xlsx(errors_all, paste0(path_errors, "errors_all_I&M.xlsx"))
```
