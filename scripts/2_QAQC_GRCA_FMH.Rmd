...
[Problem]{.underline}: Live Crown Base Height is entered incorrectly or does not make sense with other data values

[Procedure:]{.underline}

-   Sort by Status = L and DBH = Overstory. Check that LiCrBHt is [not]{.underline} blank
...
-   Sort by Status = D. Check that LiCrBHt is blank
...
-   Sort by Status = L. Check that LiCrBHt \< Ht
...
-   Filter for Status = L. Check that DamCd1, DamCd2, DamCd3, DamCd4, [and]{.underline} DamCd5 correspond to one of the choices on the FMH-1 overstory data sheets.
...
-   Filter for Status = D. Check that DamCd1, DamCd2, DamCd3, DamCd4, [and]{.underline} DamCd5 are blank.
...
if(nrow(errors_Trees_UV) > 1) {
  errors_Trees_UV <- errors_Trees_UV %>%
    filter(Error != "No Error")
} else {
  errors_Trees_UV <- errors_Trees_UV}
```
...
```{r}
errors_Shrubs_Zero <- unique(rbind())
...
[Note:]{.underline} Ignore all other order numbers (blank or \>1)

[Procedure:]{.underline}

-   Sort by Order. Check that every Order = 1 row has a RowCount = 166
...
```{r}
errors_HerbsPts_Header <- unique(rbind())

remove()

if(nrow(errors_HerbsPts_Header) > 1) {
  errors_HerbsPts_Header <- errors_HerbsPts_Header %>%
    filter(Error != "No Error")
} else {
  errors_HerbsPts_Header <- errors_HerbsPts_Header}
```

Ø  Query: Cvr_Pts_Height

[Problem:]{.underline} Height taller than 2 meters entered.

[Procedure:]{.underline}

o   Sort by Height. Check that Height ≤ 2

Ø  Query: Cvr_Pts_hits_repeat

[Problem:]{.underline} Repeated herb line measurements.

[Procedure:]{.underline}

o   Sort by RowCount. Check that RowCount = 1

Ø  Query: Cvr_Pts_OrderHt

[Problem:]{.underline} Height entered for species that is not the tallest species at a point. The tallest species should have Order = 1. All shorter species should have Order \> 1 and no height.

[Procedure:]{.underline}

o   Sort by Order. Check that all Order \>1 rows have Height = blank

[Note:]{.underline} There will be one row with Order = 1 and Height = blank

Ø  Query: Cvr_Pts_Order_repeat

[Problem:]{.underline} Repeated records for a particular order at a single point.

[Procedure:]{.underline}

o   Sort by RowCount. Check that RowCount = 1

Ø  Query: Cvr_Pts_Status
...
o   Check that Status = L or D.

o   On the Filter tab, under Species, click on Biological Not Filtered until it changes to Biological Only. Apply Selections, Sort by Status, check for annual herbaceous species with Status = D (entirely new query?).

Ø  Query: Cvr_Pts_SubHt

[Problem:]{.underline} Height entered for a substrate.

[Procedure:]{.underline}

o   Check that Height = blank (i.e. no records should appear in the query)

Ø  Query: Cvr_Pts_SubOrder

[Problem:]{.underline} Substrates recorded beneath plants or multiple substrates recorded at a point.

[Procedure:]{.underline}

o   Check that Order = 1

Ø  Query: Cvr_Pts_SubStat

[Problem:]{.underline} Substrates entered as living rather than dead.

[Procedure:]{.underline}

o   Check that Status = D

Ø  Query: Cvr_Pts_Tran
...
o   Check that Transect = 1 or 2

### HerbsPts Errors

```{r}
errors_HerbsPts <- rbind(errors_HerbsPts_Complete, errors_blank, errors_HerbsPts_Header, errors_blank, )
